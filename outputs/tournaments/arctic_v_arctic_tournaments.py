        ...
        while not done and steps < (num_steps or steps + 1):
            ...
            for agent_id in state.keys():
                if agent_id == "agent-0":
                    ...
                    if type_1 == "arctic":
                        onehot_eps = round(state["agent-1"][0]*10)
                        a_state = onehot_eps
                    else:
                        a_state = 0                     
...
if __name__ == "__main__":
    ...
    ARCTIC_AGENT = "ENTER_CHECKPOINT_OF_TRAINED_ARCTIC_AGENT"
    arctic_agent = setup_agent(ARCTIC_AGENT)

    ## arctic strategy tournament
    coop_file = open("./prisoners_arctic_vs_arctic_cooperation.csv", "a")
    coop_file.write("type_0,")
    for i in range(num_steps-1):
        coop_file.write("cooperation_{},".format(i+1))
    coop_file.write("cooperation_{}\n".format(num_steps))
    coop_file.close()
    rew_file = open("./{}_arctic_vs_arctic_rewards.csv".format(matrix_game), "a")
    rew_file.write("type_0,type_1,rewards_0,rewards_1\n")
    rew_file.close()
    for i in range(num_trials):
        c_1, r_1, r_2 = rollout(arctic_agent, "arctic", arctic_agent, "arctic")
        rew_file = open("./{}_arctic_vs_arctic_rewards.csv".format(matrix_game), "a")
        rew_file.write("arctic,arctic,{},{}\n".format(r_1,r_2))
        rew_file.close()
        coop_file = open("./{}_arctic_vs_arctic_cooperation.csv".format(matrix_game), "a")
        coop_file.write("arctic,")
